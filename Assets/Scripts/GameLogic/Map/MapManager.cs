...
namespace GameLogic.Map
{
    public class MapManager : MonoBehaviour
    {
        ...
        // PathId + Progress → 월드 좌표 (progress 0.0 = A성, 1.0 = B성)
        public Vector2 GetWorldPosition(ushort pathId, float progress)
        {
            ...
            if (!_castlePositions.TryGetValue(path.castleAId, out var castleA)) return Vector2.zero;
            if (!_castlePositions.TryGetValue(path.castleBId, out var castleB)) return Vector2.zero;

            return Vector2.Lerp(castleA, castleB, progress);
        }
    }
}